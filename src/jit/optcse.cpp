...
public:
    ...
    GenTree* ExtractSideEffectsAndCSEDefs(GenTree* expr)
    {
        ...
        public:
            ...
            SideEffectExtractor(Compiler* compiler)
                : GenTreeVisitor(compiler), m_sideEffects(compiler->getAllocator(CMK_ArrayStack))
            {
            }
            ...
            fgWalkResult PreOrderVisit(GenTree** use, GenTree* user)
            {
                ...
                if (m_compiler->gtTreeHasSideEffects(node, GTF_PERSISTENT_SIDE_EFFECTS_IN_CSE))
                {
                    if (m_compiler->gtNodeHasSideEffects(node, GTF_PERSISTENT_SIDE_EFFECTS_IN_CSE) ||
                        node->OperIs(GT_LOCKADD, GT_XADD, GT_XCHG, GT_CMPXCHG))
                    {
                        // This node has side effects so it will be kept, no need to look for
                        // CSE defs to keep and no need to decrement lclvar ref counts.
                ...
                if (node->OperIs(GT_LCL_VAR))
                {
                    ...
                    LclVarDsc* varDsc = &m_compiler->lvaTable[node->AsLclVar()->GetLclNum()];
        ...
        SideEffectExtractor extractor(m_pCompiler);
        ...
        {
            list = m_pCompiler->gtBuildCommaList(list, extractor.m_sideEffects.Pop());
        }
...
#define QQQ_CHECK_CSE_VNS 0
        ...
        do
        {
            ...
            if (IS_CSE_USE(exp->gtCSEnum))
            {
                ...
                GenTree* sideEffList = ExtractSideEffectsAndCSEDefs(exp);
