    ...
    for (BasicBlock* block = fgFirstBB; block; block = block->bbNext)
    {
        ...
        for (GenTreePtr stmt = block->bbTreeList; stmt; stmt = stmt->gtNext)
        {
            ...
            for (GenTreePtr tree = stmt->gtStmt.gtStmtList; tree; tree = tree->gtNext)
            {
                ...
                if (tree->GeneratesAssertion())
                {
                    ...
                    optImpliedAssertions(info.GetIndex(), valueGen);
                    BitVecOps::AddElemD(apTraits, valueGen, info.GetIndex() - 1);
                }
            }
        }
        ...
        if (jtrue != nullptr)
        {
            ...
            if (jtrue->GeneratesAssertion())
            {
                ...
                {
                    valueAssertionIndex    = info.GetIndex();
                    jumpDestAssertionIndex = optFindComplementary(info.GetIndex());
                }
                else
                {
                    valueAssertionIndex    = optFindComplementary(info.GetIndex());
                    jumpDestAssertionIndex = info.GetIndex();
                }
        ...
        while (stmt)
        {
            ...
            for (GenTreePtr tree = stmt->gtStmt.gtStmtList; tree; tree = tree->gtNext)
            {
                ...
                JITDUMP("Propagating %s assertions for BB%02d, stmt [%06d], tree [%06d], tree -> %d\n",
                        ...
                        tree->GetAssertionInfo().GetIndex());
                ...
                {
                    AssertionInfo info = tree->GetAssertionInfo();
                    optImpliedAssertions(info.GetIndex(), assertions);
                    BitVecOps::AddElemD(apTraits, assertions, info.GetIndex() - 1);
                }
            }
