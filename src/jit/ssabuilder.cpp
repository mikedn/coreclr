...
#ifdef DEBUG
    unsigned seqNum = 1;
    for (GenTree* node = stmt->AsStmt()->gtStmtList; node != nullptr; node = node->gtNext)
    {
    ...
    BasicBlock* block, GenTreeStmt* stmt, GenTreePhi* phi, unsigned lclNum, unsigned ssaNum, BasicBlock* pred)
{
    ...
    {
        assert(use.Node()->AsPhiArg()->GetSsaNum() != ssaNum);
    }
#endif // DEBUG
    ...
    GenTree* head = stmt->AsStmt()->gtStmtList;
            ...
            if (VarSetOps::IsMember(m_pCompiler, handler->bbLiveIn, lclIndex))
            {
                ...
                for (GenTreeStmt* stmt = handler->firstStmt(); stmt != nullptr; stmt = stmt->getNextStmt())
                {
                    ...
                    if (tree->gtOp.gtOp1->gtLclVar.gtLclNum == lclNum)
                    {
                        ...
                        AddPhiArg(handler, stmt->AsStmt(), tree->gtGetOp2()->AsPhi(), lclNum, ssaNum, block);
#ifdef DEBUG
                        phiFound = true;
#endif
                        break;
...
void SsaBuilder::AssignPhiNodeRhsVariables(BasicBlock* block, SsaRenameState* pRenameState)
{
    for (BasicBlock* succ : block->GetAllSuccs(m_pCompiler))
    {
             ...
             stmt              = stmt->getNextStmt())
        {
            ...
            for (GenTreePhi::Use& use : phi->Uses())
            {
                if (use.Node()->AsPhiArg()->GetSsaNum() == ssaNum)
                {
                    found = true;
                    break;
                }
            ...
            if (!found)
            {
                AddPhiArg(succ, stmt->AsStmt(), phi, lclNum, ssaNum, block);
            }
        }
        ...
        if (m_pCompiler->bbIsTryBeg(succ))
        {
            ...
            while (tryInd != EHblkDsc::NO_ENCLOSING_INDEX)
            {
                ...
                for (GenTreeStmt* stmt = handlerStart->firstStmt(); stmt != nullptr; stmt = stmt->getNextStmt())
                {
                    ...
                    if (!alreadyArg)
                    {
                        AddPhiArg(handlerStart, stmt->AsStmt(), phi, lclNum, ssaNum, block);
                    }
                }
