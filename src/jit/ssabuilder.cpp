...
int SsaBuilder::TopologicalSort(BasicBlock** postOrder, int count)
{
    ...
    auto DumpBlockAndSuccessors = [](Compiler* comp, BasicBlock* block) {
        ...
        if (comp->verboseSsa)
        {
            ...
            unsigned index = 0;
            for (AllSuccessorIterPosition pos(comp, block); pos.HasCurrent(); pos.Advance(comp, block))
            {
                printf("%s%sBB%02u", (index++ ? ", " : ""), (pos.IsCurrentEH() ? "[EH]" : ""),
                       pos.Current(comp, block)->bbNum);
            }
            printf("]\n");
        }
#endif
