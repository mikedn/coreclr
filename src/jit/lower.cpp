...
Lowering::SparseRangeRegion::BitRange::BitRange() : bits(1), lowerLimit(0), upperLimit(0)
{
}

Lowering::SparseRangeRegion::BitRange::BitRange(GenTreeIntCon* icon)
    : bits(1), lowerLimit(static_cast<size_t>(icon->IconValue())), upperLimit(static_cast<size_t>(icon->IconValue()))
{
}
...
bool Lowering::SparseRangeRegion::BitRange::TryAddValue(GenTreeIntCon* icon)
{
    ...
    if (value < lowerLimit)
    {
        if (upperLimit - value > 63)
        {
            return false;
        }

        bits <<= (lowerLimit - value);
        lowerLimit = value;
    }
...
Lowering::SparseRangeRegion::SparseRangeRegion(Compiler* compiler) : compiler(compiler), regionLength(0)
{
}

void Lowering::SparseRangeRegion::AddBlock(Lowering* lowering, BasicBlock* block)
{
    if (regionLength == 0)
    {
...
bool Lowering::SparseRangeRegion::TryStartRegion(BasicBlock* block)
{
    ...
    GenTreeOp* cmp = jtrue->AsUnOp()->gtGetOp1()->AsOp();

    if (!cmp->OperIs(GT_EQ) || !cmp->gtGetOp1()->OperIs(GT_LCL_VAR) || !cmp->gtGetOp2()->OperIs(GT_CNS_INT))
    {
        return false;
    }
...
bool Lowering::SparseRangeRegion::TryExpandRegion(BasicBlock* block)
{
    ...
    if (!cmp->OperIs(GT_EQ, GT_NE) || !cmp->gtGetOp1()->OperIs(GT_LCL_VAR) || !cmp->gtGetOp2()->OperIs(GT_CNS_INT))
    {
        return false;
    }
...
bool Lowering::SparseRangeRegion::CanExecuteUnconditionally(GenTree* from)
{
    ...
    {
        if (!prev->OperIs(GT_IL_OFFSET, GT_LCL_VAR, GT_CNS_INT) && !prev->OperIsCompare())
        {
            return false;
        }
    }
...
bool Lowering::SparseRangeRegion::OptimizeRegion(Lowering* lowering)
{
    if (regionLength < 3)
    {
        return false;
    ...
    GenTreeLclVar* val = cmpEntry->gtGetOp1()->AsLclVar();

    if (range.lowerLimit != 0)
    {
        LIR::AsRange(regionEntry).Remove(val);
        val->ClearContained();

        GenTree* lowerLimitOp = compiler->gtNewIconNode(range.lowerLimit, val->TypeGet());
        GenTree* sub          = compiler->gtNewOperNode(GT_SUB, val->TypeGet(), val, lowerLimitOp);
        cmpEntry->gtOp1       = sub;

        LIR::AsRange(regionEntry).InsertBefore(cmpEntry, val, lowerLimitOp, sub);
        lowering->ContainCheckBinary(sub->AsOp());

        LIR::Use use(LIR::AsRange(regionEntry), &cmpEntry->gtOp1, cmpEntry);
        val = lowering->ReplaceWithLclVar(use);
    }
    ...
    cmpEntry->gtGetOp2()->ClearContained();
    lowering->ContainCheckCompare(cmpEntry);

    GenTreeLclVar* bitIndex = cmpExit->gtGetOp1()->AsLclVar();
    bitIndex->SetLclNum(val->GetLclNum());
    bitIndex->ClearContained();

    GenTreeIntCon* bits = cmpExit->gtGetOp2()->AsIntCon();
    bits->SetIconValue(range.bits);
    bits->gtType = (range.bits > UINT32_MAX) ? TYP_LONG : TYP_INT;
    bits->ClearContained();

    // GenTree* shift = compiler->gtNewOperNode(GT_RSZ, TYP_LONG, bits, cmpExit->gtGetOp1());
    // GenTree* one = compiler->gtNewIconNode(1, TYP_LONG);
    // GenTree* bt = compiler->gtNewOperNode(GT_BT, TYP_LONG, bits, cmpExit->gtGetOp1());

    cmpExit->SetOper(GT_BT);
    cmpExit->gtFlags |= GTF_SET_FLAGS;
    cmpExit->gtType = TYP_VOID;
    cmpExit->gtOp1  = bits;
    cmpExit->gtOp2  = bitIndex;

    GenTree* jtrue = regionExit->lastNode();
    jtrue->ChangeOper(GT_JCC);
    GenTreeCC* jcc = jtrue->AsCC();
    jcc->gtFlags |= GTF_USE_FLAGS;
    jcc->gtCondition = GenCondition::NC;
    ...
    if (regionJump != regionNext)
    {
        ...
        jcc->gtCondition = GenCondition::C;
    }
    else
    {
...
bool Lowering::TryRangeCompare(BasicBlock* block, GenTreeOp* cmp)
{
    // We're looking for code like
    // if (3 < x && x < 10) { ... }
    ...
    BasicBlock* pred = block->GetUniquePred(comp);

    if ((pred == nullptr) || (pred->bbJumpKind != BBJ_COND) || (pred->bbNext != block))
    {
        return false;
    }
    ...
    if (pred->bbJumpDest == block->bbJumpDest)
    {
        inverted = false;
    }
    else if (pred->bbJumpDest == block->bbNext)
    {
        inverted = true;
    }
    else
    ...
    GenTree* predJTrue = pred->lastNode();

    if (!predJTrue->OperIs(GT_JTRUE))
    {
        // It looks like the predecessor has been lowered to something other than GT_JTRUE.
        return false;
    }

    GenTree* predCmp = predJTrue->AsUnOp()->gtGetOp1()->AsOp();
    ...
    if (!predCmpOp1->OperIs(GT_LCL_VAR) || !predCmpOp2->OperIs(GT_CNS_INT))
    {
        return false;
    }
    ...
    {
        if (!prev->OperIs(GT_IL_OFFSET, GT_LCL_VAR, GT_CNS_INT))
        {
            return false;
        }
    }
    ...
    if (lLimit != 0)
    {
        LIR::AsRange(block).Remove(cmpOp1);
        LIR::AsRange(block).Remove(cmpOp2);

        uLimitOp->SetIconValue(uLimit - lLimit);
        lLimitOp->SetIconValue(-lLimit);

        cmpOp1->ClearContained();
        lLimitOp->ClearContained();
        uLimitOp->ClearContained();

        GenTree* add = comp->gtNewOperNode(GT_ADD, cmpOp1->TypeGet(), cmpOp1, lLimitOp);
        cmp->gtOp1   = add;
        cmp->gtOp2   = uLimitOp;

        LIR::AsRange(block).InsertBefore(cmp, cmpOp1, lLimitOp, add, uLimitOp);
...
void Lowering::DoPhase()
{
    ...
    SparseRangeRegion region(comp);

    for (BasicBlock* block = comp->fgFirstBB; block != nullptr; block = block->bbNext)
    {
        if ((block->bbJumpKind == BBJ_COND) && block->lastNode()->OperIs(GT_JTRUE))
        {
            GenTreeUnOp* jtrue = block->lastNode()->AsUnOp();
            GenTreeOp*   cmp   = jtrue->gtGetOp1()->AsOp();

            if (cmp->OperIs(GT_LT, GT_LE, GT_GT, GT_GE) && cmp->gtGetOp1()->OperIs(GT_LCL_VAR) &&
                cmp->gtGetOp2()->OperIs(GT_CNS_INT))
            {
                TryRangeCompare(block, cmp);
            }
        }

        region.AddBlock(this, block);
    }
